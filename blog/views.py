...
def serialize_post(post):
    return {
        ...
        'slug': post.slug,
        'tags': [serialize_tag(tag) for tag in post.tags.fetch_with_posts()],
...
def index(request):
    ...
    most_fresh_posts = list(Post.objects
                            .order_by('published_at')
                            .prefetch_related('author')
                            .prefetch_tags()
                            .fetch_with_comments())[-5:]
...
def post_detail(request, slug):
    ...
    related_tags = post.tags.fetch_with_posts()\
    ...
    most_popular_posts = most_popular_posts = Post.objects\
        .popular()[:5]\
        .prefetch_related('author')\
        .fetch_with_comments()\
